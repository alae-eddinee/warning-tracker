...
export const ENSEIGNES = [...new Set(CLIENTS_DATA.map(c => c.enseigne))].sort();
...
export async function seedDatabase(supabaseUrl: string, supabaseKey: string) {
  ...
  // Insert stores
  console.log(`Inserting ${CLIENTS_DATA.length} stores...`);
  const batchSize = 100;
  for (let i = 0; i < CLIENTS_DATA.length; i += batchSize) {
    ...
    if (storeError) {
      ...
      console.log(`Inserted batch ${i / batchSize + 1}/${Math.ceil(CLIENTS_DATA.length / batchSize)}`);
    }
  }
